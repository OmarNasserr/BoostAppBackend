...
class DivisionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        return SerializerHelper.to_representation(
            self=self, instance=instance,
            fields_to_be_decrypted=[],
            fields_to_be_encrypted=['id', 'game_id', 'previous_division', 'parent_id']
        )


class ParentDivisionSerializer(serializers.ModelSerializer):
    division_icon = DivisionIconSerializer(many=True, read_only=True)

    class Meta:
        model = Division
        exclude = ('created_at', 'updated_at', 'previous_division', 'price', 'rank', 'parent_id')
